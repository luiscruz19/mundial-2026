  ...
  const MatchCard = ({ m, big }: { m: Match; big?: boolean }) => {
    ...
    return (
      <Pressable
        onPress={
          simulable
            ? () => navigation.navigate('Simular', { id: String(m.id) })
            : () => navigation.navigate('Match', { id: String(m.id) })
        }
        style={{
        ...
        <>
          ...
          <View style={{ paddingTop: 8, paddingHorizontal: 16 }}>
            <Card style={{ flexDirection: 'row', alignItems: 'center', gap: 14, overflow: 'hidden' }}>
              ...
              <Btn variant="ghost" size="sm" onPress={() => navigation.navigate('Tabs', { screen: 'Proyeccion' })}>Ver</Btn>
            </Card>
          </View>
        </>
      )}
